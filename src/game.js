...
blockDifficult.addEventListener("click", (event) => {
    ...
    const buttonsLevel = blockDifficult.querySelectorAll(
        ".difficult__button-level"
    );
...
btnStart.addEventListener("click", () => {
    if (
        window.game.DIFFICULT ||
        (!(window.game.DIFFICULT === "") &&
            !(window.game.DIFFICULT === undefined))
    ) {
    ...
    for (const buttonLevel of buttonsLevel) {
        ...
        setTimeout(
            () => buttonLevel.classList.remove("difficult__button-level_alert"),
            1500
        );
    }
});
